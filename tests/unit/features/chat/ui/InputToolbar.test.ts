...
describe('ModelSelector', () => {
  ...
  it('should create a container with model-selector class', () => {
    const container = parentEl.querySelector('.praetor-model-selector');
  ...
  it('should display current model label', () => {
    ...
    const btn = parentEl.querySelector('.praetor-model-btn');
    expect(btn).not.toBeNull();
    const label = btn?.querySelector('.praetor-model-label');
  ...
  it('should display the selected provider icon before the model label', () => {
    ...
    const btn = parentEl.querySelector('.praetor-model-btn');
    const icon = btn?.querySelector('.praetor-model-provider-icon');
    const label = btn?.querySelector('.praetor-model-label');
  ...
  it('should prefer the selected model provider icon in a mixed-provider picker', () => {
    ...
    const icon = parentEl.querySelector('.praetor-model-btn')
      ?.querySelector('.praetor-model-provider-icon');
  ...
  it('should display first model when current model not found', () => {
    ...
    selector.updateDisplay();
    const label = parentEl.querySelector('.praetor-model-label');
    expect(label?.textContent).toBe('Haiku');
  });

  it('should render model options in reverse order', () => {
    const dropdown = parentEl.querySelector('.praetor-model-dropdown');
  ...
  it('should render group separators when models have group field', () => {
    ...
    const groups = children.filter((c: any) => c.hasClass('praetor-model-group'));
  ...
  it('should show 1M variants instead of standard variants when enabled', () => {
    ...
    expect(parentEl.querySelector('.praetor-model-label')?.textContent).toBe('Opus 1M');
  });
});
...
describe('ModeSelector', () => {
  ...
  it('should create a container with mode-selector class', () => {
    const container = parentEl.querySelector('.praetor-mode-selector');
    expect(container).not.toBeNull();
  });

  it('should display the current mode label', () => {
    const label = parentEl.querySelector('.praetor-mode-label');
    expect(label?.textContent).toBe('Build');
  });

  it('should call onModeChange when the toggle is clicked', async () => {
    const toggle = parentEl.querySelector('.praetor-toggle-switch');
  ...
  it('should show the active style when the configured active mode is selected', () => {
    ...
    const label = parentEl2.querySelector('.praetor-mode-label');
    const toggle = parentEl2.querySelector('.praetor-toggle-switch');
    expect(label?.textContent).toBe('Build');
...
describe('ThinkingBudgetSelector', () => {
  ...
  describe('legacy mode (custom models)', () => {
    ...
    it('should render budget options in reverse order', () => {
      const options = parentEl.querySelector('.praetor-thinking-options');
...
describe('PermissionToggle', () => {
  ...
  it('should create a container with permission-toggle class', () => {
    const container = parentEl.querySelector('.praetor-permission-toggle');
    expect(container).not.toBeNull();
  });

  it('should display Safe label when in normal mode', () => {
    const label = parentEl.querySelector('.praetor-permission-label');
  ...
  it('should display YOLO label when in yolo mode', () => {
    ...
    const label = parentEl2.querySelector('.praetor-permission-label');
  ...
  it('should hide the control when provider exposes no permission toggle UI', () => {
    ...
    const container = parentEl2.querySelector('.praetor-permission-toggle');
    expect(container?.style.display).toBe('none');
  });
...
describe('ServiceTierToggle', () => {
  ...
  it('shows the control when the provider exposes service tier options', () => {
    const container = parentEl.querySelector('.praetor-service-tier-toggle');
    expect(container).not.toBeNull();
    expect(container?.hasClass('praetor-hidden')).toBe(false);
  });

  it('renders the icon button in the inactive state when fast mode is off', () => {
    const button = parentEl.querySelector('.praetor-service-tier-button');
    const icon = parentEl.querySelector('.praetor-service-tier-icon');
    const container = parentEl.querySelector('.praetor-service-tier-toggle');
  ...
  it('renders the icon button in the active state when fast mode is on', () => {
    ...
    const button = parentEl2.querySelector('.praetor-service-tier-button');
    const container = parentEl2.querySelector('.praetor-service-tier-toggle');
...
describe('McpServerSelector', () => {
  ...
  it('should create container with mcp-selector class', () => {
    const container = parentEl.querySelector('.praetor-mcp-selector');
    expect(container).not.toBeNull();
  });
  ...
  it('keeps a lazy selector hidden until configured servers finish loading', async () => {
    ...
    expect(container?.hasClass('praetor-hidden')).toBe(true);
    await Promise.resolve();
    await Promise.resolve();

    expect(manager.ensureLoaded).toHaveBeenCalledTimes(1);
    expect(container?.hasClass('praetor-hidden')).toBe(false);
    expect(parentEl.querySelector('.praetor-mcp-selector-item')).not.toBeNull();
  });
  ...
  it('should show empty message when all servers are disabled', () => {
    ...
    const empty = parentEl.querySelector('.praetor-mcp-selector-empty');
  ...
  it('should show badge when more than 1 server enabled', () => {
    ...
    const badge = parentEl.querySelector('.praetor-mcp-selector-badge');
  ...
  it('should add active class to icon when servers are enabled', () => {
    ...
    const icon = parentEl.querySelector('.praetor-mcp-selector-icon');
...
describe('ContextUsageMeter', () => {
  ...
  it('should create a container with context-meter class', () => {
    const container = parentEl.querySelector('.praetor-context-meter');
    expect(container).not.toBeNull();
  });
  ...
  it('should render percentage text for responsive styling', () => {
    meter.update(makeUsage({ contextTokens: 50000, contextWindow: 200000, percentage: 25 }));
    const percent = parentEl.querySelector('.praetor-context-meter-percent');
...
describe('InputToolbarLayoutController', () => {
  ...
  it('should compact optional labels when toolbar items wrap', () => {
    ...
    expect(toolbarEl.hasClass('praetor-input-toolbar--compact')).toBe(true);
    controller.destroy();
  });
  ...
  it('should show optional labels when all toolbar items fit on one line', () => {
    ...
    toolbarEl.addClass('praetor-input-toolbar--compact');

    const controller = new InputToolbarLayoutController(toolbarEl);
    controller.refreshLayout();

    expect(toolbarEl.hasClass('praetor-input-toolbar--compact')).toBe(false);
    controller.destroy();
  });
...
describe('McpServerSelector - toggle and badges', () => {
  ...
  it('should render context-saving badge for servers with contextSaving', () => {
    ...
    const csBadge = parentEl.querySelector('.praetor-mcp-selector-cs-badge');
  ...
  it('should toggle server on mousedown and update display', () => {
    ...
    const item = parentEl.querySelector('.praetor-mcp-selector-item');
...
describe('createInputToolbar', () => {
  ...
  it('should place the mode selector after the permission toggle in toolbar order', () => {
    ...
    const permissionIndex = parentEl.children.findIndex((child: any) => child.hasClass('praetor-permission-toggle'));
    const modeIndex = parentEl.children.findIndex((child: any) => child.hasClass('praetor-mode-selector'));
