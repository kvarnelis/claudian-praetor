...
describe('NavigationSidebar', () => {
  ...
  describe('initialization', () => {
    it('should create container with correct class', () => {
      ...
      const container = parentEl.querySelector('.pocket-codex-nav-sidebar');
  ...
  describe('previous/next message navigation', () => {
    function addUserMessage(el: MockElement, offset: number): MockElement {
      const msg = el.createDiv({ cls: 'pocket-codex-message pocket-codex-message-user' });
      msg.offsetTop = offset;
      return msg;
    }

    function addAssistantMessage(el: MockElement, offset: number): MockElement {
      const msg = el.createDiv({ cls: 'pocket-codex-message pocket-codex-message-assistant' });
      msg.offsetTop = offset;
      return msg;
    }
    ...
    function getButtons(parent: MockElement) {
      const container = parent.querySelector('.pocket-codex-nav-sidebar')!;
      return {
  ...
  describe('conversation directory', () => {
    function addMessage(
      ...
      const msg = el.createDiv({ cls: `pocket-codex-message pocket-codex-message-${role}` });
      msg.offsetTop = offset;
      if (tocTitle) {
    ...
    it('should show an empty directory state when there are no user messages', () => {
      ...
      expect(directoryBtn.classList.contains('pocket-codex-hidden')).toBe(false);

      directoryBtn.click();

      const popover = parentEl.querySelector('.pocket-codex-nav-toc-popover');
      const emptyState = parentEl.querySelector('.pocket-codex-nav-toc-empty');
    ...
    it('should render directory entries for user messages only', () => {
      ...
      const popover = parentEl.querySelector('.pocket-codex-nav-toc-popover');
      const items = parentEl.querySelectorAll('.pocket-codex-nav-toc-item');
      expect(popover).not.toBeNull();
    ...
    it('should fall back to visible user message text when toc metadata is missing', () => {
      ...
      userMsg.createDiv({
        cls: 'pocket-codex-message-content',
    ...
    it('should scroll to a selected directory entry and close the directory', () => {
      ...
      expect(lastCall.behavior).toBe('smooth');
      expect(parentEl.querySelector('.pocket-codex-nav-toc-popover')).toBeNull();
    });
    ...
    it('should close the directory when the directory button is clicked again', () => {
      ...
      const directoryBtn = getDirectoryButton(parentEl);
      directoryBtn.click();
      expect(parentEl.querySelector('.pocket-codex-nav-toc-popover')).not.toBeNull();
    ...
    it('should not rebuild an open directory for assistant content mutations', () => {
      ...
      const assistantContent = assistantMsg.createDiv({ cls: 'pocket-codex-message-content' });
      ...
      const directoryBtn = getDirectoryButton(parentEl);
      directoryBtn.click();
      const originalPopover = parentEl.querySelector('.pocket-codex-nav-toc-popover');
      ...
      expect(parentEl.querySelector('.pocket-codex-nav-toc-popover')).toBe(originalPopover);
    });
    ...
    it('should refresh an open directory when a user message toc title changes', () => {
      ...
      const updatedPopover = parentEl.querySelector('.pocket-codex-nav-toc-popover');
      const items = parentEl.querySelectorAll('.pocket-codex-nav-toc-item');
  ...
  describe('destroy', () => {
    it('should remove container from DOM', () => {
      ...
      expect(parentEl.querySelector('.pocket-codex-nav-sidebar')).not.toBeNull();

      sidebar.destroy();

      expect(parentEl.querySelector('.pocket-codex-nav-sidebar')).toBeNull();
    });
  });
});
