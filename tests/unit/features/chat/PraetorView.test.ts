...
function createViewHarness(options: {
  ...
  const view = Object.create(PraetorView.prototype) as any;
...
describe('PraetorView tab controls', () => {
  it('hides the new-tab button when the tab manager is at capacity', () => {
    ...
    expect(newTabButtonEl.hasClass('praetor-hidden')).toBe(true);
  ...
  it('shows the new-tab button when another tab can be created', () => {
    ...
    newTabButtonEl.addClass('praetor-hidden');
    newTabButtonEl.setAttribute('aria-disabled', 'true');
    newTabButtonEl.setAttribute('aria-hidden', 'true');

    view.refreshTabControls();

    expect(newTabButtonEl.hasClass('praetor-hidden')).toBe(false);
  ...
  it('preserves active pending prompt siblings during same-tab input updates', () => {
    ...
    const pendingPromptEl = inputComposerEl.createDiv({ cls: 'praetor-ask-question-inline' });
    const tab = {
      id: 'tab-1',
      dom: {
        contentEl: createMockEl(),
        inputComposerEl,
        inputContainerEl: inputComposerEl.createDiv({ cls: 'praetor-input-container' }),
      },
    };
    const view = Object.create(PraetorView.prototype) as any;
...
describe('PraetorView composer input', () => {
...
describe('PraetorView Escape handling', () => {
  beforeEach(() => {
