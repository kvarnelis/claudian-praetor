...
describe('InlineEditModal - openAndWait', () => {
  ...
  it('renders accept and reject controls in the block preview', async () => {
    ...
    try {
      ...
      const actionBar = previewEl.querySelector('.pocket-codex-inline-preview-actions');
      const actionButtons = previewEl.querySelectorAll('.pocket-codex-inline-preview-action');
  ...
  it('renders markdown diff documents with block context', async () => {
    ...
    try {
      ...
      const diffBlocks = previewEl.querySelectorAll('.pocket-codex-diff-block');
      expect(diffBlocks).toHaveLength(2);
      expect(diffBlocks[0].hasClass('pocket-codex-diff-del')).toBe(true);
      expect(diffBlocks[1].hasClass('pocket-codex-diff-ins')).toBe(true);
