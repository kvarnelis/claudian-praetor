...
function clickButton(root: any, text: string): void {
  const buttons = findAllByClass(root, 'praetor-instruction-btn');
...
describe('InstructionModal', () => {
  describe('onOpen', () => {
    it('renders the raw instruction text', () => {
      ...
      const originalEl = findByClass(contentEl, 'praetor-instruction-original');
    ...
    it('starts in loading state', () => {
      ...
      const loadingEl = findByClass(contentEl, 'praetor-instruction-loading');
      expect(loadingEl).not.toBeNull();
      expect(loadingEl.hasClass('praetor-hidden')).toBe(false);

      const clarificationEl = findByClass(contentEl, 'praetor-instruction-clarification-section');
      expect(clarificationEl.hasClass('praetor-hidden')).toBe(true);

      const confirmationEl = findByClass(contentEl, 'praetor-instruction-confirmation-section');
      expect(confirmationEl.hasClass('praetor-hidden')).toBe(true);
    });
    ...
    it('renders Cancel button in loading state', () => {
      ...
      const buttons = findAllByClass(contentEl, 'praetor-instruction-btn');
  ...
  describe('showClarification', () => {
    it('transitions to clarification state', () => {
      ...
      const loadingEl = findByClass(contentEl, 'praetor-instruction-loading');
      expect(loadingEl.hasClass('praetor-hidden')).toBe(true);

      const clarificationEl = findByClass(contentEl, 'praetor-instruction-clarification-section');
      expect(clarificationEl.hasClass('praetor-hidden')).toBe(false);
    });
    ...
    it('displays the clarification text', () => {
      ...
      const clarificationTextEl = findByClass(contentEl, 'praetor-instruction-clarification');
  ...
  describe('showConfirmation', () => {
    it('transitions to confirmation state', () => {
      ...
      const confirmationEl = findByClass(contentEl, 'praetor-instruction-confirmation-section');
      expect(confirmationEl.hasClass('praetor-hidden')).toBe(false);
    });
    ...
    it('displays the refined instruction', () => {
      ...
      const refinedEl = findByClass(contentEl, 'praetor-instruction-refined');
