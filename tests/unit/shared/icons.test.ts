...
describe('createProviderIconSvg', () => {
  it('renders path-based provider icons with currentColor fill', () => {
    ...
    expect(svg.classList.contains('praetor-provider-icon')).toBe(true);
  ...
  it('renders composite provider icons with theme variants', () => {
    ...
    expect(svg.querySelector('.praetor-provider-icon-variant--light')).not.toBeNull();
    expect(svg.querySelector('.praetor-provider-icon-variant--dark')).not.toBeNull();
  });
