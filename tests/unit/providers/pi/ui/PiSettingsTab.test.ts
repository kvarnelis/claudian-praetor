...
describe('PiSettingsTab', () => {
  ...
  it('discovers models through PiModelDiscoveryService and reports failures', async () => {
    ...
    await findElement('button', 'praetor-provider-model-picker-action').dispatchMockEvent('click');
    await flushPromises();
  ...
  it('persists visible model choices and aliases', async () => {
    ...
    const aliasInput = findElement('input', 'praetor-provider-model-picker-selected-alias');
