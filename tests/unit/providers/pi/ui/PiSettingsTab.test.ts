...
describe('PiSettingsTab', () => {
  ...
  it('discovers models through PiModelDiscoveryService and reports failures', async () => {
    ...
    await findElement('button', 'claudes-codex-provider-model-picker-action').dispatchMockEvent('click');
    await flushPromises();
  ...
  it('persists visible model choices and aliases', async () => {
    ...
    const aliasInput = findElement('input', 'claudes-codex-provider-model-picker-selected-alias');
