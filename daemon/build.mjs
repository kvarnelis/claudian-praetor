/**
 * Builds pocket-codexd into a single CJS file. Mirrors the SDK import.meta patch
 * from the root esbuild.config.mjs (cjs output has no import.meta); the
 * renderer-unref patch is intentionally skipped — pocket-codexd runs in real Node.
 */
...
await esbuild.build({
  ...
  target: 'node18',
  format: 'cjs',
  outfile: path.join(daemonDir, 'dist', 'pocket-codexd.cjs'),
  alias: {
...
console.log('pocket-codexd built: daemon/dist/pocket-codexd.cjs');
