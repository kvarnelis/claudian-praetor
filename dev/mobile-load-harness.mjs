...
const VIEW_TYPE_POCKET_CODEX = 'pocket-codex-view';
...
function createBrowserGlobals() {
  ...
  const win = {
    ...
    navigator: { userAgent: "Pocket Codex mobile-load-harness", language: 'en' },
...
function createFakeApp({ withBasePath }) {
  ...
  if (withBasePath) adapter.basePath = path.join(os.tmpdir(), 'pocket-codex-harness-vault');
...
async function runPass(platformKind) {
  ...
  try {
    ...
    const plugin = new PluginClass(app, { id: 'pocket-codex', name: "Pocket Codex", version: '0.0.0' });
    await plugin.onload();
    result.onloadResolved = true;

    // Probe: view registration happened.
    const viewTypes = spies.registeredViews.map((v) => v.type);
    if (!viewTypes.includes(VIEW_TYPE_POCKET_CODEX)) {
      result.probeFailures.push(`registerView was not called with '${VIEW_TYPE_POCKET_CODEX}' (saw: ${JSON.stringify(viewTypes)})`);
    }
