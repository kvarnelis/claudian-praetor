 ...
 * all providers: Claude's Codex settings, tab manager state, and session metadata.
 *
...
export interface SharedAppStorage {
  initialize(): Promise<{ claudesCodex: Record<string, unknown> }>;
  saveClaudesCodexSettings(settings: Record<string, unknown>): Promise<void>;
   ...
   * vault-level .claudian/ compatibility folder is a hidden top-level dir Sync skips).
   */
