/**
 * Claude's Codex - Image Embed Utilities
 *
...
function createImageHtml(
  ...
  return `<span class="claudes-codex-embedded-image"><img src="${escapeHtml(src)}" alt="${alt}" loading="lazy"${style}></span>`;
}

function createFallbackHtml(wikilink: string): string {
  return `<span class="claudes-codex-embedded-image-fallback">${escapeHtml(wikilink)}</span>`;
}
