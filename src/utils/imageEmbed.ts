/**
 * Pocket Codex - Image Embed Utilities
 *
...
function createImageHtml(
  ...
  return `<span class="pocket-codex-embedded-image"><img src="${escapeHtml(src)}" alt="${alt}" loading="lazy"${style}></span>`;
}

function createFallbackHtml(wikilink: string): string {
  return `<span class="pocket-codex-embedded-image-fallback">${escapeHtml(wikilink)}</span>`;
}
