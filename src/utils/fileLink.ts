...
function createWikilink(
  ...
  return parent.createEl('a', {
    cls: 'praetor-file-link internal-link',
    text: displayText,
    attr: {
...
function repairEmptyInternalLink(app: App, link: HTMLAnchorElement): void {
  ...
  link.classList.add('praetor-file-link');
 ...
 * Handles both our custom .praetor-file-link and Obsidian's .internal-link.
 */
export function registerFileLinkHandler(
  ...
  component.registerDomEvent(container, 'click', (event: MouseEvent) => {
    ...
    const link = target.closest('.praetor-file-link, .internal-link') as HTMLAnchorElement;
  ...
  const walker = container.ownerDocument.createTreeWalker(
    ...
    NodeFilter.SHOW_TEXT,
    {
      acceptNode(node) {
        ...
        if (parent.closest('pre, code, a, .praetor-file-link, .internal-link')) {
          return NodeFilter.FILTER_REJECT;
        }
