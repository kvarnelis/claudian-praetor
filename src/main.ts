...
export default class PocketCodexPlugin extends Plugin {
  ...
  async onload() {
    ...
    try {
      ...
      // read throws and aborts tab creation on mobile. Desktop keeps Node's real process.
      ...
      // Node APIs at module-init time, so it must never load on mobile.
  ...
  async loadSettings(options: { deferNonRestoredSessionMetadata?: boolean } = {}) {
    ...
    if (
      ...
      || didMigrateLegacyDaemonAutoStart
    ) {
