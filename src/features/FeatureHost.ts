...
import type { Conversation, ConversationMeta,PraetorSettings } from '../core/types';
import type { TabData, TabId, TabManagerViewHost } from './chat/tabs/types';

type MobileDaemonStartResult =
  | { status: 'started' | 'already-running'; url: string }
  | { status: 'no-tailnet' | 'missing-vault' | 'error'; message: string };

type MobileDaemonPairingResult =
  | { status: 'pairing-open'; url: string; expiresAt: number }
  | { status: 'no-tailnet' | 'missing-vault' | 'error'; message: string };
...
export interface FeatureHost {
  ...
  setLocalDaemonHostEnabled(enabled: boolean): Promise<MobileDaemonStartResult | null>;
  openMobileDaemonPairing(): Promise<MobileDaemonPairingResult | null>;
