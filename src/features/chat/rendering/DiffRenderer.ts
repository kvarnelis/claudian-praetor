...
export function renderDiffContent(
  ...
  if (allInserts && diffLines.length > NEW_FILE_DISPLAY_CAP) {
    const hunkEl = containerEl.createDiv({ cls: 'praetor-diff-hunk' });
    for (const line of diffLines.slice(0, NEW_FILE_DISPLAY_CAP)) {
      const lineEl = hunkEl.createDiv({ cls: 'praetor-diff-line praetor-diff-insert' });
      const prefixEl = lineEl.createSpan({ cls: 'praetor-diff-prefix' });
      prefixEl.setText('+');
      const contentEl = lineEl.createSpan({ cls: 'praetor-diff-text' });
      contentEl.setText(line.text || ' ');
    }
    const remaining = diffLines.length - NEW_FILE_DISPLAY_CAP;
    const separator = containerEl.createDiv({ cls: 'praetor-diff-separator' });
  ...
  if (hunks.length === 0) {
    // No changes
    const noChanges = containerEl.createDiv({ cls: 'praetor-diff-no-changes' });
  ...
  hunks.forEach((hunk, hunkIndex) => {
    ...
    for (const line of hunk.lines) {
      const lineEl = hunkEl.createDiv({ cls: `praetor-diff-line praetor-diff-${line.type}` });
