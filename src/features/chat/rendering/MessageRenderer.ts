...
export class MessageRenderer {
  ...
  addMessage(msg: ChatMessage): HTMLElement {
    ...
    const msgEl = this.messagesEl.createDiv({
      cls: `pocket-codex-message pocket-codex-message-${msg.role}`,
      attr: {
    ...
    const contentEl = msgEl.createDiv({ cls: 'pocket-codex-message-content', attr: { dir: 'auto' } });

    if (msg.role === 'user') {
      const textToShow = this.getUserMessageTextToShow(msg);
      if (textToShow) {
        const textEl = contentEl.createDiv({ cls: 'pocket-codex-text-block' });
  ...
  updateLiveUserMessage(msg: ChatMessage): void {
    ...
    const toolbar = msgEl.querySelector<HTMLElement>('.pocket-codex-user-msg-actions');
    if (toolbar) {
      toolbar.querySelectorAll('.pocket-codex-user-msg-copy-btn').forEach((el) => el.remove());
    }
  ...
  private renderInterruptMessage(): void {
    const msgEl = this.messagesEl.createDiv({ cls: 'pocket-codex-message pocket-codex-message-assistant' });
    const contentEl = msgEl.createDiv({ cls: 'pocket-codex-message-content', attr: { dir: 'auto' } });
    this.appendInterruptIndicator(contentEl);
  }
  ...
  appendInterruptIndicator(contentEl: HTMLElement): void {
    const textEl = contentEl.createDiv({ cls: 'pocket-codex-text-block' });
    textEl.createSpan({ cls: 'pocket-codex-interrupted', text: 'Interrupted' });
    textEl.appendText(' ');
    textEl.createSpan({
      cls: 'pocket-codex-interrupted-hint',
      text: "\u00B7 What should Pocket Codex do instead?",
    });
  }
  ...
  private renderAssistantContent(msg: ChatMessage, contentEl: HTMLElement): boolean {
    ...
    if (msg.contentBlocks && msg.contentBlocks.length > 0) {
      ...
      for (const block of msg.contentBlocks) {
        if (block.type === 'thinking') {
          ...
          flushPendingCodexText();
          const boundaryEl = contentEl.createDiv({ cls: 'pocket-codex-compact-boundary' });
          boundaryEl.createSpan({ cls: 'pocket-codex-compact-boundary-label', text: 'Conversation compacted' });
    ...
    if (msg.durationSeconds && msg.durationSeconds > 0 && !hasCompactBoundary) {
      ...
      const footerEl = contentEl.createDiv({ cls: 'pocket-codex-response-footer' });
      footerEl.createSpan({
  ...
  private shouldRenderToolCall(toolCall: ToolCallInfo): boolean {
    ...
    if (this.getCapabilities().providerId === 'codex') {
      ...
      if (subagentLifecycleAdapter?.isSpawnTool(toolCall.name)) return true;
      return false;
    }
  ...
  renderMessageImages(containerEl: HTMLElement, images: ImageAttachment[]): void {
    const imagesEl = containerEl.createDiv({ cls: 'pocket-codex-message-images' });

    for (const image of images) {
      const imageWrapper = imagesEl.createDiv({ cls: 'pocket-codex-message-image' });
  ...
  showFullImage(image: ImageAttachment): void {
    ...
    const overlay = ownerDocument.body.createDiv({ cls: 'pocket-codex-image-modal-overlay' });
    const modal = overlay.createDiv({ cls: 'pocket-codex-image-modal' });
    ...
    const closeBtn = modal.createDiv({ cls: 'pocket-codex-image-modal-close' });
    closeBtn.setText('\u00D7');
  ...
  async renderContent(
    ...
    try {
      ...
      el.querySelectorAll('pre').forEach((pre) => {
        ...
        if (pre.parentElement?.classList.contains('pocket-codex-code-wrapper')) return;

        // Create wrapper
        const wrapper = createDiv({ cls: 'pocket-codex-code-wrapper' });
        ...
        if (code) {
          ...
          if (match) {
            ...
            const label = createSpan({
              cls: 'pocket-codex-code-lang-label',
              text: match[1],
            });
  ...
  addTextCopyButton(textEl: HTMLElement, markdown: string): void {
    const copyBtn = textEl.createSpan({ cls: 'pocket-codex-text-copy-btn' });
  ...
  refreshActionButtons(msg: ChatMessage, allMessages?: ChatMessage[], index?: number): void {
    ...
    if (canRewind && this.rewindCallback && !msgEl.querySelector('.pocket-codex-message-rewind-btn')) {
      this.addRewindButton(msgEl, msg.id);
    }
    if (canFork && this.forkCallback && !msgEl.querySelector('.pocket-codex-message-fork-btn')) {
      this.addForkButton(msgEl, msg.id);
    }
  ...
  private cleanupLiveMessageEl(
    ...
    const needsRewind = expectedActions.canRewind
      && this.rewindCallback
      && !msgEl.querySelector('.pocket-codex-message-rewind-btn');
    const needsFork = expectedActions.canFork
      && this.forkCallback
      && !msgEl.querySelector('.pocket-codex-message-fork-btn');
  ...
  private getOrCreateActionsToolbar(msgEl: HTMLElement): HTMLElement {
    const existing = msgEl.querySelector<HTMLElement>('.pocket-codex-user-msg-actions');
    if (existing) return existing;
    return msgEl.createDiv({ cls: 'pocket-codex-user-msg-actions' });
  }

  private addUserCopyButton(msgEl: HTMLElement, content: string): void {
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const copyBtn = toolbar.createSpan({ cls: 'pocket-codex-user-msg-copy-btn' });
    setIcon(copyBtn, 'copy');
  ...
  private addRewindButton(msgEl: HTMLElement, messageId: string): void {
    ...
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const btn = toolbar.createSpan({ cls: 'pocket-codex-message-rewind-btn' });
  ...
  private addForkButton(msgEl: HTMLElement, messageId: string): void {
    ...
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const btn = toolbar.createSpan({ cls: 'pocket-codex-message-fork-btn' });
    if (toolbar.firstChild !== btn) toolbar.insertBefore(btn, toolbar.firstChild);
