...
export class MessageRenderer {
  ...
  addMessage(msg: ChatMessage): HTMLElement {
    ...
    const msgEl = this.messagesEl.createDiv({
      cls: `claudes-codex-message claudes-codex-message-${msg.role}`,
      attr: {
    ...
    const contentEl = msgEl.createDiv({ cls: 'claudes-codex-message-content', attr: { dir: 'auto' } });

    if (msg.role === 'user') {
      const textToShow = this.getUserMessageTextToShow(msg);
      if (textToShow) {
        const textEl = contentEl.createDiv({ cls: 'claudes-codex-text-block' });
  ...
  updateLiveUserMessage(msg: ChatMessage): void {
    ...
    const toolbar = msgEl.querySelector<HTMLElement>('.claudes-codex-user-msg-actions');
    if (toolbar) {
      toolbar.querySelectorAll('.claudes-codex-user-msg-copy-btn').forEach((el) => el.remove());
    }
  ...
  private renderInterruptMessage(): void {
    const msgEl = this.messagesEl.createDiv({ cls: 'claudes-codex-message claudes-codex-message-assistant' });
    const contentEl = msgEl.createDiv({ cls: 'claudes-codex-message-content', attr: { dir: 'auto' } });
    this.appendInterruptIndicator(contentEl);
  }
  ...
  appendInterruptIndicator(contentEl: HTMLElement): void {
    const textEl = contentEl.createDiv({ cls: 'claudes-codex-text-block' });
    textEl.createSpan({ cls: 'claudes-codex-interrupted', text: 'Interrupted' });
    textEl.appendText(' ');
    textEl.createSpan({
      cls: 'claudes-codex-interrupted-hint',
      text: "\u00B7 What should Claude's Codex do instead?",
    });
  }
  ...
  private renderAssistantContent(msg: ChatMessage, contentEl: HTMLElement): boolean {
    ...
    if (msg.contentBlocks && msg.contentBlocks.length > 0) {
      ...
      for (const block of msg.contentBlocks) {
        if (block.type === 'thinking') {
          ...
          flushPendingCodexText();
          const boundaryEl = contentEl.createDiv({ cls: 'claudes-codex-compact-boundary' });
          boundaryEl.createSpan({ cls: 'claudes-codex-compact-boundary-label', text: 'Conversation compacted' });
    ...
    if (msg.durationSeconds && msg.durationSeconds > 0 && !hasCompactBoundary) {
      ...
      const footerEl = contentEl.createDiv({ cls: 'claudes-codex-response-footer' });
      footerEl.createSpan({
  ...
  renderMessageImages(containerEl: HTMLElement, images: ImageAttachment[]): void {
    const imagesEl = containerEl.createDiv({ cls: 'claudes-codex-message-images' });

    for (const image of images) {
      const imageWrapper = imagesEl.createDiv({ cls: 'claudes-codex-message-image' });
  ...
  async renderContent(
    ...
    try {
      ...
      el.querySelectorAll('pre').forEach((pre) => {
        ...
        if (pre.parentElement?.classList.contains('claudes-codex-code-wrapper')) return;

        // Create wrapper
        const wrapper = createDiv({ cls: 'claudes-codex-code-wrapper' });
        ...
        if (code) {
          ...
          if (match) {
            ...
            const label = createSpan({
              cls: 'claudes-codex-code-lang-label',
              text: match[1],
            });
      ...
      el.createDiv({
        cls: 'claudes-codex-render-error',
  ...
  addTextCopyButton(textEl: HTMLElement, markdown: string): void {
    const copyBtn = textEl.createSpan({ cls: 'claudes-codex-text-copy-btn' });
  ...
  refreshActionButtons(msg: ChatMessage, allMessages?: ChatMessage[], index?: number): void {
    ...
    if (canRewind && this.rewindCallback && !msgEl.querySelector('.claudes-codex-message-rewind-btn')) {
      this.addRewindButton(msgEl, msg.id);
    }
    if (canFork && this.forkCallback && !msgEl.querySelector('.claudes-codex-message-fork-btn')) {
      this.addForkButton(msgEl, msg.id);
    }
  ...
  private cleanupLiveMessageEl(
    ...
    const needsRewind = expectedActions.canRewind
      && this.rewindCallback
      && !msgEl.querySelector('.claudes-codex-message-rewind-btn');
    const needsFork = expectedActions.canFork
      && this.forkCallback
      && !msgEl.querySelector('.claudes-codex-message-fork-btn');
  ...
  private getOrCreateActionsToolbar(msgEl: HTMLElement): HTMLElement {
    const existing = msgEl.querySelector<HTMLElement>('.claudes-codex-user-msg-actions');
    if (existing) return existing;
    return msgEl.createDiv({ cls: 'claudes-codex-user-msg-actions' });
  }

  private addUserCopyButton(msgEl: HTMLElement, content: string): void {
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const copyBtn = toolbar.createSpan({ cls: 'claudes-codex-user-msg-copy-btn' });
    setIcon(copyBtn, 'copy');
  ...
  private addRewindButton(msgEl: HTMLElement, messageId: string): void {
    ...
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const btn = toolbar.createSpan({ cls: 'claudes-codex-message-rewind-btn' });
  ...
  private addForkButton(msgEl: HTMLElement, messageId: string): void {
    ...
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const btn = toolbar.createSpan({ cls: 'claudes-codex-message-fork-btn' });
    if (toolbar.firstChild !== btn) toolbar.insertBefore(btn, toolbar.firstChild);
