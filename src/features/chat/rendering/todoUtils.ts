...
export function renderTodoItems(
  ...
  for (const todo of todos) {
    const item = container.createDiv({ cls: `praetor-todo-item praetor-todo-${todo.status}` });

    const icon = item.createSpan({ cls: 'praetor-todo-status-icon' });
    icon.setAttribute('aria-hidden', 'true');
    setIcon(icon, getTodoStatusIcon(todo.status));

    const text = item.createSpan({ cls: 'praetor-todo-text' });
