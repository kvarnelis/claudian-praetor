...
export function createThinkingBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'praetor-thinking-block' });

  // Header (clickable to expand/collapse)
  const header = wrapperEl.createDiv({ cls: 'praetor-thinking-header' });
  ...
  // Label with timer
  const labelEl = header.createSpan({ cls: 'praetor-thinking-label' });
  ...
  const contentEl = wrapperEl.createDiv({ cls: 'praetor-thinking-content' });
...
export function finalizeThinkingBlock(state: ThinkingBlockState): number {
  ...
  const header = state.wrapperEl.querySelector('.praetor-thinking-header');
  if (header) {
