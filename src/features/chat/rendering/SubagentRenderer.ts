...
function createSection(parentEl: HTMLElement, title: string, bodyClass?: string): SubagentSection {
  const wrapperEl = parentEl.createDiv({ cls: 'pocket-codex-subagent-section' });

  const headerEl = wrapperEl.createDiv({ cls: 'pocket-codex-subagent-section-header' });
  headerEl.setAttribute('tabindex', '0');
  headerEl.setAttribute('role', 'button');

  const titleEl = headerEl.createDiv({ cls: 'pocket-codex-subagent-section-title' });
  titleEl.setText(title);

  const bodyEl = wrapperEl.createDiv({ cls: 'pocket-codex-subagent-section-body' });
...
function setPromptText(promptBodyEl: HTMLElement, prompt: string): void {
  promptBodyEl.empty();
  const textEl = promptBodyEl.createDiv({ cls: 'pocket-codex-subagent-prompt-text' });
...
function renderSubagentToolContent(contentEl: HTMLElement, toolCall: ToolCallInfo): void {
  ...
  if (!toolCall.result && toolCall.status === 'running') {
    const emptyEl = contentEl.createDiv({ cls: 'pocket-codex-subagent-tool-empty' });
...
function setSubagentToolStatus(view: SubagentToolView, status: ToolCallInfo['status']): void {
  view.statusEl.className = 'pocket-codex-subagent-tool-status';
...
function updateSubagentToolView(view: SubagentToolView, toolCall: ToolCallInfo): void {
  view.wrapperEl.className = `pocket-codex-subagent-tool-item pocket-codex-subagent-tool-${toolCall.status}`;
...
function createSubagentToolView(parentEl: HTMLElement, toolCall: ToolCallInfo): SubagentToolView {
  const wrapperEl = parentEl.createDiv({
    cls: `pocket-codex-subagent-tool-item pocket-codex-subagent-tool-${toolCall.status}`,
  });
  wrapperEl.dataset.toolId = toolCall.id;

  const headerEl = wrapperEl.createDiv({ cls: 'pocket-codex-subagent-tool-header' });
  headerEl.setAttribute('tabindex', '0');
  headerEl.setAttribute('role', 'button');

  const iconEl = headerEl.createDiv({ cls: 'pocket-codex-subagent-tool-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setToolIcon(iconEl, toolCall.name);

  const nameEl = headerEl.createDiv({ cls: 'pocket-codex-subagent-tool-name' });
  const summaryEl = headerEl.createDiv({ cls: 'pocket-codex-subagent-tool-summary' });
  const statusEl = headerEl.createDiv({ cls: 'pocket-codex-subagent-tool-status' });

  const contentEl = wrapperEl.createDiv({ cls: 'pocket-codex-subagent-tool-content' });
...
function ensureResultSection(state: SubagentState): SubagentSection {
  ...
  const section = createSection(state.contentEl, 'Result', 'pocket-codex-subagent-result-body');
  section.wrapperEl.addClass('pocket-codex-subagent-section-result');
...
function setResultText(state: SubagentState, text: string): void {
  ...
  const resultEl = section.bodyEl.createDiv({ cls: 'pocket-codex-subagent-result-output' });
...
function hydrateSyncSubagentStateFromStored(state: SubagentState, subagent: SubagentInfo): void {
  ...
  if (subagent.status === 'completed' || subagent.status === 'error') {
    ...
    state.statusEl.className = 'pocket-codex-subagent-status status-running';
...
export function createSubagentBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'pocket-codex-subagent-list' });
  wrapperEl.dataset.subagentId = taskToolId;

  const headerEl = wrapperEl.createDiv({ cls: 'pocket-codex-subagent-header' });
  headerEl.setAttribute('tabindex', '0');
  headerEl.setAttribute('role', 'button');

  const iconEl = headerEl.createDiv({ cls: 'pocket-codex-subagent-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setIcon(iconEl, getToolIcon(TOOL_TASK));

  const labelEl = headerEl.createDiv({ cls: 'pocket-codex-subagent-label' });
  labelEl.setText(truncateDescription(description));

  const statusEl = headerEl.createDiv({ cls: 'pocket-codex-subagent-status status-running' });
  statusEl.setAttribute('aria-label', 'Status: running');

  const contentEl = wrapperEl.createDiv({ cls: 'pocket-codex-subagent-content' });

  const promptSection = createSection(contentEl, 'Prompt', 'pocket-codex-subagent-prompt-body');
  promptSection.wrapperEl.addClass('pocket-codex-subagent-section-prompt');
  setPromptText(promptSection.bodyEl, prompt);

  const toolsContainerEl = contentEl.createDiv({ cls: 'pocket-codex-subagent-tools' });
...
export function finalizeSubagentBlock(
  ...
  state.statusEl.className = 'pocket-codex-subagent-status';
...
function renderAsyncContentLikeSync(
  ...
  const resultSection = createSection(contentEl, 'Result', 'pocket-codex-subagent-result-body');
  resultSection.wrapperEl.addClass('pocket-codex-subagent-section-result');
  const resultEl = resultSection.bodyEl.createDiv({ cls: 'pocket-codex-subagent-result-output' });
...
export function createAsyncSubagentBlock(
  ...
  const statusTextEl = headerEl.createDiv({ cls: 'pocket-codex-subagent-status-text' });
...
export function markAsyncSubagentOrphaned(state: AsyncSubagentState): void {
  ...
  state.statusEl.className = 'pocket-codex-subagent-status status-error';
  state.statusEl.empty();
...
export function renderStoredAsyncSubagent(
  ...
  const statusEl = headerEl.createDiv({ cls: `pocket-codex-subagent-status ${statusIconClass}` });
