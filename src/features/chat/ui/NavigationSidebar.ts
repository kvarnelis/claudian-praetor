...
export class NavigationSidebar {
  ...
  constructor(
    ...
    this.container = this.parentEl.createDiv({ cls: 'pocket-codex-nav-sidebar' });

    // Create buttons
    this.topBtn = this.createButton('pocket-codex-nav-btn-top', 'chevrons-up', 'Scroll to top');
    this.prevBtn = this.createButton('pocket-codex-nav-btn-prev', 'chevron-up', 'Previous message');
    this.tocBtn = this.createButton('pocket-codex-nav-btn-toc', 'list-tree', 'Conversation directory');
    this.nextBtn = this.createButton('pocket-codex-nav-btn-next', 'chevron-down', 'Next message');
    this.bottomBtn = this.createButton('pocket-codex-nav-btn-bottom', 'chevrons-down', 'Scroll to bottom');
  ...
  private createButton(cls: string, icon: string, label: string): HTMLElement {
    const btn = this.container.createDiv({ cls: `pocket-codex-nav-btn ${cls}` });
    setIcon(btn, icon);
  ...
  private applyVisibility(): void {
    ...
    this.tocBtn.classList.remove('pocket-codex-hidden');
  ...
  private getDirectoryEntries(): Array<{ el: HTMLElement; title: string }> {
    return Array.from(this.messagesEl.querySelectorAll<HTMLElement>('.pocket-codex-message-user, [data-role="user"]'))
      .map(el => ({
        el,
  ...
  private getDirectoryTitle(el: HTMLElement): string {
    ...
    const contentEl = el.querySelector<HTMLElement>('.pocket-codex-message-content');
  ...
  private nodeContainsDirectoryMessage(node: Node): boolean {
    ...
    return typeof candidate.querySelector === 'function'
      && candidate.querySelector('.pocket-codex-message-user, [data-role="user"]') !== null;
  }
  ...
  private isDirectoryMessageElement(node: Node): boolean {
    ...
    if (typeof candidate.matches === 'function') {
      return candidate.matches('.pocket-codex-message-user, [data-role="user"]');
    }
    return candidate.classList?.contains?.('pocket-codex-message-user') === true
  ...
  private openDirectory(): void {
    ...
    this.closeDirectory();
    this.tocPopover = this.parentEl.createDiv({ cls: 'pocket-codex-nav-toc-popover' });
    this.tocPopover.createDiv({ cls: 'pocket-codex-nav-toc-title', text: 'Conversation directory' });
    const listEl = this.tocPopover.createDiv({ cls: 'pocket-codex-nav-toc-list' });

    if (entries.length === 0) {
      listEl.createDiv({
        cls: 'pocket-codex-nav-toc-empty',
  ...
  private scrollToMessage(direction: 'prev' | 'next'): void {
    const messages = Array.from(this.messagesEl.querySelectorAll<HTMLElement>('.pocket-codex-message-user'));
