...
export class ModelSelector {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    ...
    this.container = parentEl.createDiv({ cls: 'pocket-codex-model-selector' });
    this.render();
  ...
  private render() {
    ...
    this.buttonEl = this.container.createDiv({ cls: 'pocket-codex-model-btn' });
    this.updateDisplay();

    this.dropdownEl = this.container.createDiv({ cls: 'pocket-codex-model-dropdown' });
    this.renderOptions();
  }
  ...
  updateDisplay() {
    ...
    if (icon) {
      createProviderIconSvg(icon, {
        className: 'pocket-codex-model-provider-icon',
        height: 12,
        parent: this.buttonEl,
        width: 12,
      });
    }
    const labelEl = this.buttonEl.createSpan({ cls: 'pocket-codex-model-label' });
  ...
  renderOptions() {
    ...
    for (const model of reversed) {
      if (model.group && model.group !== lastGroup) {
        const separator = this.dropdownEl.createDiv({ cls: 'pocket-codex-model-group' });
        separator.setText(model.group);
        lastGroup = model.group;
      }

      const option = this.dropdownEl.createDiv({ cls: 'pocket-codex-model-option' });
...
export class ModeSelector {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    this.callbacks = callbacks;
    this.container = parentEl.createDiv({ cls: 'pocket-codex-mode-selector' });
    this.render();
  }
  ...
  private render() {
    ...
    this.labelEl = this.container.createSpan({ cls: 'pocket-codex-mode-label' });
    this.toggleEl = this.container.createDiv({ cls: 'pocket-codex-toggle-switch' });
  ...
  updateDisplay() {
    ...
    if (!selectorConfig || selectorConfig.options.length !== 2) {
      this.container.addClass('pocket-codex-hidden');
      return;
    }

    this.container.removeClass('pocket-codex-hidden');
...
export class ThinkingBudgetSelector {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    this.callbacks = callbacks;
    this.container = parentEl.createDiv({ cls: 'pocket-codex-thinking-selector' });
    this.render();
  }
  ...
  private render() {
    ...
    this.effortEl = this.container.createDiv({ cls: 'pocket-codex-thinking-effort' });
    const effortLabel = this.effortEl.createSpan({ cls: 'pocket-codex-thinking-label-text' });
    effortLabel.setText('Effort:');
    this.effortGearsEl = this.effortEl.createDiv({ cls: 'pocket-codex-thinking-gears' });

    // Legacy budget selector (for custom models)
    this.budgetEl = this.container.createDiv({ cls: 'pocket-codex-thinking-budget' });
    const budgetLabel = this.budgetEl.createSpan({ cls: 'pocket-codex-thinking-label-text' });
    budgetLabel.setText('Thinking:');
    this.budgetGearsEl = this.budgetEl.createDiv({ cls: 'pocket-codex-thinking-gears' });
  ...
  private renderEffortGears() {
    ...
    const currentEl = this.effortGearsEl.createDiv({ cls: 'pocket-codex-thinking-current' });
    currentEl.setText(currentInfo?.label || options[0]?.label || 'High');

    const optionsEl = this.effortGearsEl.createDiv({ cls: 'pocket-codex-thinking-options' });

    for (const effort of [...options].reverse()) {
      const gearEl = optionsEl.createDiv({ cls: 'pocket-codex-thinking-gear' });
  ...
  private renderBudgetGears() {
    ...
    const currentEl = this.budgetGearsEl.createDiv({ cls: 'pocket-codex-thinking-current' });
    currentEl.setText(currentBudgetInfo?.label || options[0]?.label || 'Off');

    const optionsEl = this.budgetGearsEl.createDiv({ cls: 'pocket-codex-thinking-options' });
  ...
  updateDisplay() {
    ...
    if (capabilities.reasoningControl === 'none') {
      this.effortEl?.addClass('pocket-codex-hidden');
      this.budgetEl?.addClass('pocket-codex-hidden');
      return;
    }
    ...
    if (this.effortEl) {
      this.effortEl.toggleClass('pocket-codex-hidden', !adaptive);
    }
    if (this.budgetEl) {
      this.budgetEl.toggleClass('pocket-codex-hidden', adaptive);
    }
...
export class PermissionToggle {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    this.callbacks = callbacks;
    this.container = parentEl.createDiv({ cls: 'pocket-codex-permission-toggle' });
    this.render();
  }
  ...
  private render() {
    ...
    this.labelEl = this.container.createSpan({ cls: 'pocket-codex-permission-label' });
    this.toggleEl = this.container.createDiv({ cls: 'pocket-codex-toggle-switch' });
  ...
  updateDisplay() {
    ...
    if (canShowPlan && planValue && mode === planValue) {
      this.toggleEl.addClass('pocket-codex-hidden');
      this.labelEl.setText(planLabel);
      this.labelEl.addClass('plan-active');
    } else {
      this.toggleEl.removeClass('pocket-codex-hidden');
...
export class ServiceTierToggle {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    this.callbacks = callbacks;
    this.container = parentEl.createDiv({ cls: 'pocket-codex-service-tier-toggle' });
    this.render();
  }
  ...
  private render() {
    ...
    this.buttonEl = this.container.createDiv({ cls: 'pocket-codex-service-tier-button' });
    this.iconEl = this.buttonEl.createSpan({ cls: 'pocket-codex-service-tier-icon' });
...
export class ExternalContextSelector {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    this.callbacks = callbacks;
    this.container = parentEl.createDiv({ cls: 'pocket-codex-external-context-selector' });
    this.render();
  }
  ...
  private render() {
    ...
    const iconWrapper = this.container.createDiv({ cls: 'pocket-codex-external-context-icon-wrapper' });

    this.iconEl = iconWrapper.createDiv({ cls: 'pocket-codex-external-context-icon' });
    setIcon(this.iconEl, 'folder');

    this.badgeEl = iconWrapper.createDiv({ cls: 'pocket-codex-external-context-badge' });
    ...
    this.dropdownEl = this.container.createDiv({ cls: 'pocket-codex-external-context-dropdown' });
    this.renderDropdown();
  }
  ...
  private renderDropdown() {
    ...
    // Header
    const headerEl = this.dropdownEl.createDiv({ cls: 'pocket-codex-external-context-header' });
    headerEl.setText('External contexts');

    // Path list
    const listEl = this.dropdownEl.createDiv({ cls: 'pocket-codex-external-context-list' });

    if (this.externalContextPaths.length === 0) {
      const emptyEl = listEl.createDiv({ cls: 'pocket-codex-external-context-empty' });
      emptyEl.setText('Click folder icon to add');
    } else {
      for (const pathStr of this.externalContextPaths) {
        const itemEl = listEl.createDiv({ cls: 'pocket-codex-external-context-item' });

        const pathTextEl = itemEl.createSpan({ cls: 'pocket-codex-external-context-text' });
        ...
        const lockBtn = itemEl.createSpan({ cls: 'pocket-codex-external-context-lock' });
        if (isPersistent) {
        ...
        const removeBtn = itemEl.createSpan({ cls: 'pocket-codex-external-context-remove' });
        setIcon(removeBtn, 'x');
...
export class McpServerSelector {
  ...
  constructor(parentEl: HTMLElement) {
    this.container = parentEl.createDiv({ cls: 'pocket-codex-mcp-selector' });
    this.render();
  }
  ...
  private render() {
    ...
    const iconWrapper = this.container.createDiv({ cls: 'pocket-codex-mcp-selector-icon-wrapper' });

    this.iconEl = iconWrapper.createDiv({ cls: 'pocket-codex-mcp-selector-icon' });
    appendMcpIcon(this.iconEl);

    this.badgeEl = iconWrapper.createDiv({ cls: 'pocket-codex-mcp-selector-badge' });

    this.updateDisplay();

    this.dropdownEl = this.container.createDiv({ cls: 'pocket-codex-mcp-selector-dropdown' });
    this.renderDropdown();
  ...
  private renderDropdown() {
    ...
    // Header
    const headerEl = this.dropdownEl.createDiv({ cls: 'pocket-codex-mcp-selector-header' });
    headerEl.setText('Mcp servers');

    // Server list
    const listEl = this.dropdownEl.createDiv({ cls: 'pocket-codex-mcp-selector-list' });

    const allServers = this.mcpManager?.getServers() || [];
    const servers = allServers.filter(s => s.enabled);

    if (servers.length === 0) {
      const emptyEl = listEl.createDiv({ cls: 'pocket-codex-mcp-selector-empty' });
  ...
  private renderServerItem(listEl: HTMLElement, server: ManagedMcpServer) {
    const itemEl = listEl.createDiv({ cls: 'pocket-codex-mcp-selector-item' });
    ...
    // Checkbox
    const checkEl = itemEl.createDiv({ cls: 'pocket-codex-mcp-selector-check' });
    if (isEnabled) {
      appendCheckIcon(checkEl);
    }

    // Info
    const infoEl = itemEl.createDiv({ cls: 'pocket-codex-mcp-selector-item-info' });

    const nameEl = infoEl.createSpan({ cls: 'pocket-codex-mcp-selector-item-name' });
    nameEl.setText(server.name);

    // Badges
    if (server.contextSaving) {
      const csEl = infoEl.createSpan({ cls: 'pocket-codex-mcp-selector-cs-badge' });
      csEl.setText('@');
  ...
  private toggleServer(name: string, itemEl: HTMLElement) {
    ...
    const checkEl = itemEl.querySelector<HTMLElement>('.pocket-codex-mcp-selector-check');
...
export class ContextUsageMeter {
  ...
  constructor(parentEl: HTMLElement) {
    this.container = parentEl.createDiv({ cls: 'pocket-codex-context-meter' });
  ...
  setVisible(visible: boolean): void {
    this.container.toggleClass('pocket-codex-hidden', !visible);
  }
  ...
  private render() {
    ...
    const gaugeEl = this.container.createDiv({ cls: 'pocket-codex-context-meter-gauge' });
    ...
    backgroundPath.classList.add('pocket-codex-meter-bg');
    ...
    fillPath.classList.add('pocket-codex-meter-fill');
    ...
    this.percentEl = this.container.createSpan({ cls: 'pocket-codex-context-meter-percent' });
  }
...
const TOOLBAR_COMPACT_CLASS = 'pocket-codex-input-toolbar--compact';
