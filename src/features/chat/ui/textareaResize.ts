...
export function autoResizeTextarea(textarea: HTMLTextAreaElement): void {
  const viewHeight = textarea.closest('.praetor-container')?.clientHeight ?? window.innerHeight;
  const maxHeight = calculateTextareaMaxHeight(viewHeight);

  textarea.setCssProps({
    '--praetor-textarea-min-height': `${TEXTAREA_BASE_MIN_HEIGHT}px`,
    '--praetor-textarea-max-height': `${maxHeight}px`,
  });
  ...
  textarea.setCssProps({
    '--praetor-textarea-min-height': `${minHeight}px`,
    '--praetor-textarea-max-height': `${maxHeight}px`,
  });
}
