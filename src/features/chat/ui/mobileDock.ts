...
const BODY_CLASS = 'pocket-codex-ipad-docked';
const DRAWER_CLASS = 'pocket-codex-dock-drawer';
const ROOT_CLASS = 'pocket-codex-dock-root';
 ...
 * editor (split)" placement on a tablet we keep Pocket Codex in the right drawer
 ...
 */
const DOCK_WIDTH_KEY = 'pocket-codex-dock-width';
...
export class MobileDock {
  ...
  private restorePersistedWidth(): void {
    try {
      ...
      if (saved) document.body.style.setProperty('--pocket-codex-dock-width', saved);
    } catch {
  ...
  private ensureHandle(): void {
    ...
    const handle = document.body.createDiv({ cls: 'pocket-codex-dock-handle' });
    ...
    handle.addEventListener('pointermove', (e) => {
      ...
      document.body.style.setProperty('--pocket-codex-dock-width', `${Math.round(width)}px`);
    });
    const end = (e: PointerEvent): void => {
      ...
      try {
        ...
        const value = document.body.style.getPropertyValue('--pocket-codex-dock-width');
