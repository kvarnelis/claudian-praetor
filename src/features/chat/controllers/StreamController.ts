...
export class StreamController {
  ...
  private handleRegularToolUse(
    ...
    if (existingToolCall) {
      ...
      if (Object.keys(newInput).length > 0) {
        ...
        if (toolEl) {
          const nameEl = toolEl.querySelector('.praetor-tool-name')
            ?? toolEl.querySelector('.praetor-write-edit-name');
          if (nameEl) {
            nameEl.setText(getToolName(existingToolCall.name, existingToolCall.input));
          }
          const summaryEl = toolEl.querySelector('.praetor-tool-summary')
            ?? toolEl.querySelector('.praetor-write-edit-summary');
          if (summaryEl) {
  ...
  async appendText(text: string): Promise<void> {
    ...
    if (!state.currentTextEl) {
      state.currentTextEl = state.currentContentEl.createDiv({ cls: 'praetor-text-block' });
  ...
  showThinkingIndicator(overrideText?: string, overrideCls?: string): void {
    ...
    state.setThinkingIndicatorTimeout(timerWindow.setTimeout(() => {
      ...
      const timerSpan = state.thinkingEl.createSpan({ cls: 'praetor-thinking-hint' });
      const updateTimer = () => {
  ...
  private renderCompactBoundary(): void {
    ...
    if (!state.currentContentEl) return;
    this.hideThinkingIndicator();
    const el = state.currentContentEl.createDiv({ cls: 'praetor-compact-boundary' });
    el.createSpan({ cls: 'praetor-compact-boundary-label', text: 'Conversation compacted' });
  }
