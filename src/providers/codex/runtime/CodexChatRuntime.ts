...
const LEGACY_WORKSPACE_DEPENDENCY_NOTICE =
  "This conversation was created before Pocket Codex added Codex workspace dependency tools. It can continue for other tasks, but skills that require load_workspace_dependencies are unavailable in this thread. Start a new conversation to use them.";

const LEGACY_WORKSPACE_DEPENDENCY_INSTRUCTIONS =
  "This thread predates Pocket Codex client-hosted workspace dependency tools. If the user requests a skill that requires load_workspace_dependencies, explain that they must start a new conversation in Pocket Codex. Do not emulate the tool, search for dependency paths, or install replacement dependencies.";
  ...
  private buildInput(text: string, images?: ImageAttachment[], skills?: SkillInput[]): CodexInputBundle {
    ...
    try {
      if (images && images.length > 0) {
        tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pocket-codex-codex-images-'));
