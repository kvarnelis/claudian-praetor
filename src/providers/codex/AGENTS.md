...
- `CodexNotificationRouter` projects normalized notifications and raw response items into Claude's Codex `StreamChunk`s.
...
- `CodexTaskResultInterpreter` is intentionally no-op because Claude's Codex's Claude async-agent task system does not apply to Codex.
