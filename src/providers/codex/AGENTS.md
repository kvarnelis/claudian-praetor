...
- `CodexNotificationRouter` projects normalized notifications and raw response items into Pocket Codex `StreamChunk`s.
...
- `CodexTaskResultInterpreter` is intentionally no-op because Pocket Codex's Claude async-agent task system does not apply to Codex.
