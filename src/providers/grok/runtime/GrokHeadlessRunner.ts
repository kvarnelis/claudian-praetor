...
function writeGrokPromptFile(prompt: string): string {
  const filePath = path.join(
    os.tmpdir(),
    `praetor-grok-${Date.now()}-${Math.random().toString(36).slice(2)}.txt`,
  );
...
## Grok Build Praetor Integration
The vault's CLAUDE.md "Startup Greeting" rule applies only to the first assistant reply in a new Praetor conversation. This is a resumed conversation, so do not repeat startup ASCII art, Ajman cat art, NETLAB banners, or dry one-line welcomes. Answer the user's current message directly using the existing conversation context.`;
}

export function buildGrokTurnPrompt(prompt: string, isFollowupTurn: boolean): string {
  if (!isFollowupTurn) return prompt;
  return `<praetor_followup_reminder>
This is a follow-up turn in an existing Praetor conversation using Grok Build session resume. Continue the prior conversation and answer the user's current message directly.
Do not run startup behavior on this turn. Do not include startup ASCII art, Ajman cat art, NETLAB banners, dry one-line welcomes, or first-run/session-start rituals. Do not claim Praetor, Grok Build, or claude-anywhere has no carryover memory between prompts.
</praetor_followup_reminder>
${prompt}`;
}
