...
- `CCSettingsStorage.save()` must merge with existing `.claude/settings.json`; Pocket Codex only owns permissions and plugin enablement.
- `.claude/mcp.json` has a Claude-compatible `mcpServers` namespace and a Pocket Codex `_claudian.servers` metadata namespace. Keep them separate.
