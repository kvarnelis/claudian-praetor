...
export class PluginSettingsManager {
  ...
  private render() {
    ...
    const headerEl = this.containerEl.createDiv({ cls: 'pocket-codex-plugin-header' });
    headerEl.createSpan({ text: 'Claude Code Plugins', cls: 'pocket-codex-plugin-label' });
    ...
    if (plugins.length === 0) {
      const emptyEl = this.containerEl.createDiv({ cls: 'pocket-codex-plugin-empty' });
    ...
    const listEl = this.containerEl.createDiv({ cls: 'pocket-codex-plugin-list' });

    if (projectPlugins.length > 0) {
      const sectionHeader = listEl.createDiv({ cls: 'pocket-codex-plugin-section-header' });
  ...
  private renderPluginItem(listEl: HTMLElement, plugin: PluginInfo) {
    const itemEl = listEl.createDiv({ cls: 'pocket-codex-plugin-item' });
    if (!plugin.enabled) {
      itemEl.addClass('pocket-codex-plugin-item-disabled');
    }

    const statusEl = itemEl.createDiv({ cls: 'pocket-codex-plugin-status' });
    if (plugin.enabled) {
      statusEl.addClass('pocket-codex-plugin-status-enabled');
    } else {
      statusEl.addClass('pocket-codex-plugin-status-disabled');
    }

    const infoEl = itemEl.createDiv({ cls: 'pocket-codex-plugin-info' });

    const nameRow = infoEl.createDiv({ cls: 'pocket-codex-plugin-name-row' });

    const nameEl = nameRow.createSpan({ cls: 'pocket-codex-plugin-name' });
    nameEl.setText(plugin.name);

    const actionsEl = itemEl.createDiv({ cls: 'pocket-codex-plugin-actions' });

    const toggleBtn = actionsEl.createEl('button', {
      cls: 'pocket-codex-plugin-action-btn',
