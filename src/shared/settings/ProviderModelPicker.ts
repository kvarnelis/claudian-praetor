...
export function renderProviderModelPicker(
  ...
  const pickerEl = options.container.createDiv({
    cls: `pocket-codex-provider-model-picker pocket-codex-provider-model-picker--${options.modifier}`,
  });
  let searchQuery = '';
  ...
  const summaryEl = pickerEl.createDiv({ cls: 'pocket-codex-provider-model-picker-summary' });
  const selectedEl = pickerEl.createDiv({ cls: 'pocket-codex-provider-model-picker-selected' });
  const catalogEl = pickerEl.createEl('details', { cls: 'pocket-codex-provider-model-picker-catalog' });
  ...
  const catalogSummaryEl = catalogEl.createEl('summary', {
    cls: 'pocket-codex-provider-model-picker-catalog-summary',
  });
  catalogSummaryEl.createSpan({
    cls: 'pocket-codex-provider-model-picker-catalog-caret',
    text: '▸',
  });
  catalogSummaryEl.createSpan({
    cls: 'pocket-codex-provider-model-picker-catalog-title',
    text: 'Browse models',
  });
  const catalogSummaryCountEl = catalogSummaryEl.createSpan({
    cls: 'pocket-codex-provider-model-picker-catalog-count',
  });

  const controlsEl = catalogEl.createDiv({ cls: 'pocket-codex-provider-model-picker-controls' });
  const searchInput = controlsEl.createEl('input', {
    cls: 'pocket-codex-provider-model-picker-search',
    type: 'search',
  });
  ...
  const providerSelectEl = controlsEl.createEl('select', {
    cls: 'pocket-codex-provider-model-picker-provider',
  });
  ...
  const catalogActionEl = controlsEl.createEl('button', {
    cls: 'pocket-codex-provider-model-picker-action',
    text: 'Discover',
  });
  ...
  const listEl = catalogEl.createDiv({ cls: 'pocket-codex-provider-model-picker-list' });
  ...
  const renderSummary = (): void => {
    ...
    summaryEl.createSpan({
      cls: 'pocket-codex-provider-model-picker-summary-value',
  ...
  const renderSelected = (): void => {
    ...
    if (state.selectedIds.length === 0) {
      selectedEl.toggleClass('pocket-codex-hidden', true);
      return;
    }

    selectedEl.toggleClass('pocket-codex-hidden', false);
    const modelsById = new Map(state.models.map(model => [model.id, model] as const));
    const headerEl = selectedEl.createDiv({ cls: 'pocket-codex-provider-model-picker-selected-header' });
    headerEl.createSpan({
      cls: 'pocket-codex-provider-model-picker-selected-label',
      text: `Selected (${state.selectedIds.length})`,
    });
    const clearAllButton = headerEl.createEl('button', {
      cls: 'pocket-codex-provider-model-picker-selected-clear',
      text: 'Clear all',
    });
    ...
    const rowsEl = selectedEl.createDiv({ cls: 'pocket-codex-provider-model-picker-selected-rows' });
    for (const modelId of state.selectedIds) {
      ...
      const rowEl = rowsEl.createDiv({ cls: 'pocket-codex-provider-model-picker-selected-row' });
      if (model.isAvailable === false) {
        rowEl.classList.add('pocket-codex-provider-model-picker-selected-row--unavailable');
      }

      const infoEl = rowEl.createDiv({ cls: 'pocket-codex-provider-model-picker-selected-info' });
      const titleEl = infoEl.createDiv({ cls: 'pocket-codex-provider-model-picker-selected-title' });
      if (model.providerLabel) {
        titleEl.createSpan({
          cls: 'pocket-codex-provider-model-picker-selected-badge',
          text: model.providerLabel,
        });
      }
      titleEl.createSpan({
        cls: 'pocket-codex-provider-model-picker-selected-name',
        text: model.name,
      });
      if (model.isAvailable === false && model.unavailableMessage) {
        infoEl.createDiv({
          cls: 'pocket-codex-provider-model-picker-selected-unavailable',
          text: model.unavailableMessage,
        });
      }
      infoEl.createDiv({
        cls: 'pocket-codex-provider-model-picker-selected-id',
        text: model.id,
      });

      const rowControlsEl = rowEl.createDiv({ cls: 'pocket-codex-provider-model-picker-selected-controls' });
      const aliasInput = rowControlsEl.createEl('input', {
        cls: 'pocket-codex-provider-model-picker-selected-alias',
        type: 'text',
      });
      ...
      const removeButton = rowControlsEl.createEl('button', {
        cls: 'pocket-codex-provider-model-picker-selected-remove',
        text: '×',
      });
  ...
  const renderProviderSelect = (): void => {
    ...
    providerSelectEl.toggleClass('pocket-codex-hidden', providers.size === 0);
  ...
  const renderList = (): void => {
    ...
    if (models.length === 0) {
      listEl.createDiv({
        cls: 'pocket-codex-provider-model-picker-empty',
        text: loadingCatalog
    ...
    for (const model of models) {
      const rowEl = listEl.createEl('label', { cls: 'pocket-codex-provider-model-picker-row' });
      const isSelected = selectedIds.has(model.id);
      if (isSelected) {
        rowEl.classList.add('pocket-codex-provider-model-picker-row--selected');
      }
      ...
      const textEl = rowEl.createDiv({ cls: 'pocket-codex-provider-model-picker-row-text' });
      const headerEl = textEl.createDiv({ cls: 'pocket-codex-provider-model-picker-row-header' });
      headerEl.createSpan({
        cls: 'pocket-codex-provider-model-picker-row-name',
        text: model.name,
      });
      ...
      if (badgeLabel) {
        const badgeEl = headerEl.createSpan({
          cls: 'pocket-codex-provider-model-picker-row-badge',
          text: badgeLabel,
        });
        if (model.isAvailable === false) {
          badgeEl.classList.add('pocket-codex-provider-model-picker-row-badge--unavailable');
          badgeEl.title = model.unavailableTitle ?? `Configured model not currently reported by ${options.providerName}`;
        }
      }
      textEl.createDiv({
        cls: 'pocket-codex-provider-model-picker-row-meta',
        text: model.id,
      });
      if (model.description) {
        textEl.createDiv({
          cls: 'pocket-codex-provider-model-picker-row-desc',
