...
export class McpSettingsManager {
  ...
  private render() {
    ...
    const headerEl = this.containerEl.createDiv({ cls: 'praetor-mcp-header' });
    headerEl.createSpan({ text: 'MCP Servers', cls: 'praetor-mcp-label' });

    const addContainer = headerEl.createDiv({ cls: 'praetor-mcp-add-container' });
    ...
    const dropdown = addContainer.createDiv({ cls: 'praetor-mcp-add-dropdown' });

    const stdioOption = dropdown.createDiv({ cls: 'praetor-mcp-add-option' });
    setIcon(stdioOption.createSpan({ cls: 'praetor-mcp-add-option-icon' }), 'terminal');
    ...
    const httpOption = dropdown.createDiv({ cls: 'praetor-mcp-add-option' });
    setIcon(httpOption.createSpan({ cls: 'praetor-mcp-add-option-icon' }), 'globe');
    ...
    const importOption = dropdown.createDiv({ cls: 'praetor-mcp-add-option' });
    setIcon(importOption.createSpan({ cls: 'praetor-mcp-add-option-icon' }), 'clipboard-paste');
    ...
    if (this.servers.length === 0) {
      const emptyEl = this.containerEl.createDiv({ cls: 'praetor-mcp-empty' });
      emptyEl.setText('No mcp servers configured. Click "add" to add one.');
      return;
    }

    const listEl = this.containerEl.createDiv({ cls: 'praetor-mcp-list' });
  ...
  private renderServerItem(listEl: HTMLElement, server: ManagedMcpServer) {
    const itemEl = listEl.createDiv({ cls: 'praetor-mcp-item' });
    if (!server.enabled) {
      itemEl.addClass('praetor-mcp-item-disabled');
    }

    const statusEl = itemEl.createDiv({ cls: 'praetor-mcp-status' });
    statusEl.addClass(
      server.enabled ? 'praetor-mcp-status-enabled' : 'praetor-mcp-status-disabled'
    );

    const infoEl = itemEl.createDiv({ cls: 'praetor-mcp-info' });

    const nameRow = infoEl.createDiv({ cls: 'praetor-mcp-name-row' });

    const nameEl = nameRow.createSpan({ cls: 'praetor-mcp-name' });
    nameEl.setText(server.name);

    const serverType = getMcpServerType(server.config);
    const typeEl = nameRow.createSpan({ cls: 'praetor-mcp-type-badge' });
    typeEl.setText(serverType);

    if (server.contextSaving) {
      const csEl = nameRow.createSpan({ cls: 'praetor-mcp-context-saving-badge' });
      csEl.setText('@');
      csEl.setAttribute('title', 'Context-saving: mention with @' + server.name + ' to enable');
    }

    const previewEl = infoEl.createDiv({ cls: 'praetor-mcp-preview' });
    ...
    const actionsEl = itemEl.createDiv({ cls: 'praetor-mcp-actions' });

    const testBtn = actionsEl.createEl('button', {
      cls: 'praetor-mcp-action-btn',
    ...
    const deleteBtn = actionsEl.createEl('button', {
      cls: 'praetor-mcp-action-btn praetor-mcp-delete-btn',
