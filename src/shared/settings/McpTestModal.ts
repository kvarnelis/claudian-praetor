...
export class McpTestModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('pocket-codex-mcp-test-modal');
  ...
  private renderLoading() {
    ...
    const loadingEl = this.contentEl_.createDiv({ cls: 'pocket-codex-mcp-test-loading' });

    const spinnerEl = loadingEl.createDiv({ cls: 'pocket-codex-mcp-test-spinner' });
  ...
  private render() {
    ...
    const statusEl = this.contentEl_.createDiv({ cls: 'pocket-codex-mcp-test-status' });

    const iconEl = statusEl.createSpan({ cls: 'pocket-codex-mcp-test-icon' });
    ...
    const textEl = statusEl.createSpan({ cls: 'pocket-codex-mcp-test-text' });
    if (this.result.success) {
    ...
    if (this.result.error) {
      const errorEl = this.contentEl_.createDiv({ cls: 'pocket-codex-mcp-test-error' });
    ...
    if (this.result.tools.length > 0) {
      const toolsSection = this.contentEl_.createDiv({ cls: 'pocket-codex-mcp-test-tools' });

      const toolsHeader = toolsSection.createDiv({ cls: 'pocket-codex-mcp-test-tools-header' });
      toolsHeader.setText(`Available Tools (${this.result.tools.length})`);

      const toolsList = toolsSection.createDiv({ cls: 'pocket-codex-mcp-test-tools-list' });

      for (const tool of this.result.tools) {
        this.renderTool(toolsList, tool);
      }
    } else if (this.result.success) {
      const noToolsEl = this.contentEl_.createDiv({ cls: 'pocket-codex-mcp-test-no-tools' });
      noToolsEl.setText('No tools information available. Tools will be loaded when used in chat.');
    }

    const buttonContainer = this.contentEl_.createDiv({ cls: 'pocket-codex-mcp-test-buttons' });

    if (this.result.tools.length > 0 && this.onToolToggle) {
      this.toggleAllBtn = buttonContainer.createEl('button', {
        cls: 'pocket-codex-mcp-toggle-all-btn',
      });
  ...
  private renderTool(container: HTMLElement, tool: McpTool) {
    const toolEl = container.createDiv({ cls: 'pocket-codex-mcp-test-tool' });

    const headerEl = toolEl.createDiv({ cls: 'pocket-codex-mcp-test-tool-header' });

    const iconEl = headerEl.createSpan({ cls: 'pocket-codex-mcp-test-tool-icon' });
    setIcon(iconEl, 'wrench');

    const nameEl = headerEl.createSpan({ cls: 'pocket-codex-mcp-test-tool-name' });
    nameEl.setText(tool.name);

    const toggleEl = headerEl.createDiv({ cls: 'pocket-codex-mcp-test-tool-toggle' });
    ...
    if (tool.description) {
      const descEl = toolEl.createDiv({ cls: 'pocket-codex-mcp-test-tool-desc' });
  ...
  private updateToolState(toolEl: HTMLElement, enabled: boolean) {
    toolEl.toggleClass('pocket-codex-mcp-test-tool-disabled', !enabled);
  }
