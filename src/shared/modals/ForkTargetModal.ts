...
class ForkTargetModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('praetor-fork-target-modal');

    const list = this.contentEl.createDiv({ cls: 'praetor-fork-target-list' });
  ...
  private createOption(container: HTMLElement, target: ForkTarget, label: string): void {
    const item = container.createDiv({ cls: 'praetor-fork-target-option', text: label });
